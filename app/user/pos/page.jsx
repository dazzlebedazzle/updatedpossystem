...
export default function UserPOS() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      // The API will filter products where product.supplier matches user.name
                    ...
                    <div key={item.productId} className="bg-white rounded-lg p-2 border border-gray-200">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={() => {
                              const newQty = item.quantity - (item.unit === 'kg' ? 100 : 1);
                              // Allow going to 0, but not negative
                              if (newQty >= 0) {
                          ...
                          <div className="flex items-center">
                            <input
                              type="text"
                              inputMode="numeric"
                              value={item.quantity === 0 ? '' : (item.unit === 'kg' ? item.quantity : item.quantity)}
                              onChange={(e) => {
                                const inputValue = e.target.value;
                                // Allow empty string so user can clear and type
                                if (inputValue === '') {
                                ...
                                // Remove leading zeros (e.g., "030" -> "30", "0" -> "0")
                              ...
                              onFocus={(e) => {
                                // When user focuses, select all text so they can type directly
                                e.target.select();
                              }}
                              onBlur={(e) => {
                                // When user leaves the field, ensure minimum is 1 if empty or 0
                                const value = parseInt(e.target.value) || 0;
                                if (value <= 0) {
                                  updateQuantity(item.productId, item.unit === 'kg' ? 100 : 1);
                                }
                              }}
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleCheckout(); }}>
                  ...
                  <div className="flex gap-3 mt-6">
                    <button
                      ...
                      className="flex-1 bg-red-600 text-white py-2.5 px-4 rounded-lg hover:bg-red-700 disabled:bg-red-400 disabled:cursor-not-allowed font-medium transition flex items-center justify-center gap-2"
                    >
                      {isProcessingCheckout ? (
                        <>
