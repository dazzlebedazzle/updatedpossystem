...
export function sanitizeSession(session) {
  ...
  const allowedFields = ['userId', 'email', 'role', 'token', 'permissions', 'assignedShopIds', 'mustChangePassword', 'passwordExpiresAt'];
  const sanitized = {};
  ...
  for (const field of allowedFields) {
    if (session[field] !== undefined) {
      ...
      if (field === 'userId' && typeof session[field] === 'string') {
        ...
        if (['superadmin', 'admin', 'agent', 'manager'].includes(role)) {
      ...
      } else if (field === 'assignedShopIds' && Array.isArray(session[field])) {
        sanitized[field] = session[field]
          .filter((id) => typeof id === 'string')
          .slice(0, 100);
      } else if (field === 'mustChangePassword' && typeof session[field] === 'boolean') {
        sanitized[field] = session[field];
      } else if (field === 'passwordExpiresAt' && (typeof session[field] === 'string' || session[field] === null)) {
        sanitized[field] = session[field];
      }
    }
  }
